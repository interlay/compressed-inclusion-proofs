...
contract("BrokenRelay storeHeader", async (_accounts) => {
    let relay;
    ...
    it("submit 1 block after initial Genesis parent ", async () => {
        ...
        const submitBlock1 = await relay.submitBlockHeader(block.header);
    ...
    it("VerifyTx with confirmations", async () => {
        ...
        const submitBlock1 = await relay.submitBlockHeader(block1.header);
        truffleAssert.eventEmitted(submitBlock1, "StoreHeader", (ev) => {
        ...
        for (const block of testdata.slice(2, 8)) {
            await relay.submitBlockHeader(block.header);
        }
    ...
    it("VerifyTx large block", async () => {
        ...
        for (const block of testdata.slice(1, 3)) {
            relay.submitBlockHeader(block.header);
        }
        const block = testdata[3];

        const submitBlock = await relay.submitBlockHeader(block.header);
