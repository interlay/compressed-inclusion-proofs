...
contract BrokenRelay {
    ...
    */
    ...
    function validateTxChainTransaction(
        bytes memory rawTx, 
        bytes32[] memory txids)
        public {

        // parse transaction, extracting all inputs and the txids references in these inputs
        bytes memory extractedTxids = extractInputTxids(rawTx);

        // check if all txids are matching (we can assume correct ordering)
        require(extractedTxids.length.mod(32) == txids.length, "Incorrect number of txids in proof transaction");

        for(uint i = 0; i < txids.length; i++) {
            require(txids[i] == extractedTxids.slice(i*32, 32).toBytes32(), 
            "Mismatching txids in proof transaction. TXIDs need to be submitted in same order as the tx inputs.");
        }

        emit CheckTxChainProof(txids);
